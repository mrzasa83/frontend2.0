...
"[project]/frontend2.4/app/api/dashboard/team-backlog/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    ...
    try {
        // Query the amph_cc database (connection '1' based on your production endpoint)
        const query = `
      SELECT 
        ps.id, 
        j.name,
        ...
        ps.group_id,
        ps.status_id,
        CONVERT(VARCHAR, DATEADD(HOUR, -1, DATEADD(SECOND, ps.start, '1970-01-01')), 120) as startTime,
        CASE 
          WHEN ps.end IS NOT NULL AND ps.end > 0 
          THEN CONVERT(VARCHAR, DATEADD(HOUR, -1, DATEADD(SECOND, ps.end, '1970-01-01')), 120)
          ELSE NULL 
        END as endTime
      FROM procstatus ps
      INNER JOIN job j ON j.id = ps.job_id
      INNER JOIN route r ON j.route = r.id
      WHERE ps.group_id = 0 
        AND ps.proc_id = 0 
        AND j.job_type NOT LIKE 'QUOTE' 
        AND j.route != 63
        AND YEAR(DATEADD(HOUR, -1, DATEADD(SECOND, ps.start, '1970-01-01'))) >= YEAR(GETDATE()) - 3
...
//# sourceMappingURL=%5Broot-of-the-server%5D__f15c1c38._.js.map