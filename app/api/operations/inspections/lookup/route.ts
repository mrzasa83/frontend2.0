...
export async function GET(request: NextRequest) {
  ...
  try {
    // 1. Where-used to find customer parts
    const customerParts = await whereUsed(partNumber)

    // Keep only 5-digit customer part numbers
    const fiveDigit = customerParts.filter(c => /^\d{5}$/.test(c.customerPart))

    // 2. Work order search for each 5-digit customer part
    const allWorkOrders: any[] = []
    for (const cp of fiveDigit) {
