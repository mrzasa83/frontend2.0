...
const TABLE_HINT = 'Ensure the coat_department table exists (run sql/create_coat_department.sql).'
...
export async function GET() {
  ...
  try {
    const rows = await queryMySQL<{ dept_code: string; dept_name: string | null }>(
      `SELECT dept_code, dept_name FROM coat_department ORDER BY dept_code ASC`
    )
    return NextResponse.json({ departments: rows.map(r => ({ deptCode: r.dept_code, deptName: r.dept_name || '' })) })
  } catch (error) {
    console.error('coat_department GET error:', error)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await queryMySQL(
      `INSERT INTO coat_department (dept_code, dept_name, created_by) VALUES (?, ?, ?)
       ON DUPLICATE KEY UPDATE dept_name = VALUES(dept_name)`,
      [code, deptName ? String(deptName).trim() : null, user]
    )
    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('coat_department POST error:', error)
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (all === '1') {
      await queryMySQL(`DELETE FROM coat_department`)
    } else if (code) {
      await queryMySQL(`DELETE FROM coat_department WHERE dept_code = ?`, [code])
    } else {
      return NextResponse.json({ error: 'deptCode or all=1 required' }, { status: 400 })
    }
    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('coat_department DELETE error:', error)
