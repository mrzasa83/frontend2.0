...
import {
  parseCmmSheet, computeStats, histogram, rayleighCurve, tpFrom,
  type FeatureRow,
} from '@/lib/process/drillRoutPpk'

type Meta = { part: string; machine: string; spindle: string; date: string }

const fmt = (v: number, d = 5) => (isFinite(v) ? v.toFixed(d) : '—')

export default function DrillRoutPpkPage() {
  ...
  const domain = useMemo(() => {
    ...
    // True position starts at zero; there is no meaningful negative side.
    let lo = 0
    let hi = Math.max(dataHi, stats.mean + 4 * stats.sd)
    if (axisMode === 'spec') hi = Math.max(hi, usl)
    const pad = (hi - lo) * 0.02
    return [lo - pad, hi + pad] as [number, number]
  }, [bins, stats, usl, axisMode])

  // Bars are the measured buckets; the curve is the right-skewed (Rayleigh) fit
  // drawn from zero out past the spec limit.
  const histoData = useMemo(() => {
    if (!bins.length || !stats) return []
    const binWidth = bins[0].end - bins[0].start
    const curve = rayleighCurve(stats.rayleighSigma ?? 0, stats.n, binWidth, domain[0], domain[1], 160)

    // Bars and curve share one count axis. Measured values pile onto repeated
    // readings (the CMM rounds to 4 decimals), so the bars run taller than any
    // smooth fit. Scale the drawn curve so its peak meets the tallest bucket —
    // the honest expected-count value is kept for the tooltip.
    const peakBar = Math.max(...bins.map(b => b.count), 0)
    const peakCurve = Math.max(...curve.map((c: { curve: number }) => c.curve), 0)
    const scale = peakCurve > 0 && peakBar > 0 ? peakBar / peakCurve : 1

    const merged: { x: number; count?: number; curve?: number; expected?: number }[] =
      curve.map((c: { x: number; curve: number }) => ({ x: c.x, curve: c.curve * scale, expected: c.curve }))
  ...
  const curveScale = useMemo(() => {
    ...
    const binWidth = bins[0].end - bins[0].start
    const c = rayleighCurve(stats.rayleighSigma ?? 0, stats.n, binWidth, domain[0], domain[1], 160)
    const peakBar = Math.max(...bins.map(b => b.count), 0)
  ...
  const exportExcel = async () => {
    ...
    const summary = [
      ...
      ['USL (tolerance)', stats.usl],
      ['Ppk (USL, overall sigma)', stats.ppk ?? ''],
      ['Ppk (Rayleigh percentile)', stats.ppkPercentile ?? ''],
      ['Rayleigh sigma', stats.rayleighSigma ?? ''],
      ['Sigma level', stats.sigmaLevel ?? ''],
      ['Above USL', stats.aboveUsl],
    ...
    const slug = [meta.part, meta.machine, meta.spindle].filter(Boolean).join('_').replace(/[^\w.-]+/g, '-') || 'drill-rout-ppk'
    XLSX.writeFile(wb, `${slug}_ppk_${(meta.date || new Date().toISOString().slice(0, 10)).replace(/\//g, '-')}.xlsx`)
  }
  ...
  return (
    <div className="p-6">
      <div className="mb-5">
        <h1 className="text-2xl font-bold text-slate-800">Drill/Rout Ppk Calc</h1>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
              ...
              <div className="xl:col-span-2 space-y-4">
                ...
                <Panel title="Histogram — true position (TP)"
                  subtitle={`${bins.length} buckets across the measured range · right-skewed (Rayleigh) fit against USL ${usl}`}>
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <ComposedChart data={histoData} margin={{ top: 10, right: 20, bottom: 30, left: 10 }}>
                      ...
                      <Bar dataKey="count" name="Measured" barSize={barSize} isAnimationActive={false}>
                        {histoData.map((d, i) => (
                          <Cell key={i} fill={d.x > usl ? '#dc2626' : '#2563eb'} />
                        ))}
                      </Bar>
                      <Line type="monotone" dataKey="curve" name="Rayleigh fit (scaled to peak)" stroke="#0f766e"
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Bars are the measured buckets; the curve is a Rayleigh distribution fitted to the data —
                    the right-skewed shape true position actually takes, since it is a distance bounded at zero.
                    Both sit on the same count axis, with the curve scaled ×{curveScale.toFixed(1)} so its shape is
                    comparable to the bars — hover any point for the unscaled expected count.
                    {axisMode === 'data' && usl > domain[1] && (
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-5">
                  ...
                  <div className="text-center py-3 border-y border-slate-100 mb-3">
                    <div className="text-4xl font-bold tabular-nums text-slate-800">
                      {stats.ppk !== null ? stats.ppk.toFixed(2) : '—'}
                    </div>
                    <div className="text-xs uppercase tracking-wide text-slate-400 mt-1">
                      Ppk — one-sided to USL, overall sigma
                    </div>
                  </div>
                  <dl className="space-y-1.5 text-sm">
                    ...
                    <Stat label="Ppk (Rayleigh %ile)" value={stats.ppkPercentile !== null ? stats.ppkPercentile.toFixed(2) : '—'} />
                    <Stat label="Rayleigh sigma" value={stats.rayleighSigma !== null ? fmt(stats.rayleighSigma, 6) : '—'} />
                    <Stat label="Sigma level" value={stats.sigmaLevel !== null ? stats.sigmaLevel.toFixed(2) : '—'} />
                    <Stat label="Above USL" value={String(stats.aboveUsl)}
                      danger={stats.aboveUsl > 0} />
                  </dl>
                </div>
