...
export type Stats = {
  ...
  range: number
  usl: number
  ppk: number | null            // (USL - mean) / 3s, overall sigma
  ppu: number | null            // same figure, named for the upper limit
  sigmaLevel: number | null
  outOfSpec: number
  aboveUsl: number
  rayleighSigma: number | null  // fitted scale of the right-skewed distribution
  ppkPercentile: number | null  // distribution-aware Ppk (ISO 22514 percentile form)
  meanX: number
  meanY: number
  sdX: number
  sdY: number
...
export function computeStats(rows: FeatureRow[], usl: number): Stats {
  ...
  // Ppk uses the OVERALL (long-term) standard deviation, which is what the
  // sample sd above is. True position is one-sided: it is a distance, bounded
  // at zero, so only the upper limit constrains it.
  const ppu = sd > 0 ? (usl - mean) / (3 * sd) : null

  // Distribution-aware alternative. True position is Rayleigh-shaped, not
  // normal, so the classic 3-sigma form understates the tail. The percentile
  // form (ISO 22514) uses the fitted distribution instead:
  //     Ppk = (USL - x50) / (x99.865 - x50)
  const rSigma = rayleighSigmaFit(tps)
  const ppkPercentile = rSigma
    ? (usl - rayleighQuantile(rSigma, 0.5)) /
      (rayleighQuantile(rSigma, 0.99865) - rayleighQuantile(rSigma, 0.5))
    : null
  ...
  return {
    ...
    usl,
    ppk: ppu, ppu,
    sigmaLevel: ppu !== null ? ppu * 3 : null,
    outOfSpec: aboveUsl,
    aboveUsl,
    rayleighSigma: rSigma,
    ppkPercentile,
    meanX, meanY, sdX, sdY,
  }
}
...
/**
 * Fit the Rayleigh scale parameter by method of moments.
 * For TP = sqrt(dx^2 + dy^2) with roughly equal, centred x/y scatter, the
 * distribution is Rayleigh: sigma^2 = (1 / 2n) * sum(x^2).
 */
export function rayleighSigmaFit(values: number[]): number | null {
  if (!values.length) return null
  const s2 = values.reduce((a, v) => a + v * v, 0) / (2 * values.length)
  const s = Math.sqrt(s2)
  return isFinite(s) && s > 0 ? s : null
}

/** Rayleigh quantile: x_p = sigma * sqrt(-2 ln(1 - p)). */
export function rayleighQuantile(sigma: number, p: number): number {
...
/**
 * Points for a Rayleigh curve fitted to the data, scaled to the histogram's
 * counts so the two share one axis. Height = pdf(x) * n * binWidth.
 *
 * True position is the length of a two-axis deviation vector, so it is bounded
 * at zero and skewed right — a normal curve would spill below zero and
 * understate the upper tail. Rayleigh is the distribution that shape implies.
 */
export function rayleighCurve(
  sigma: number, n: number, binWidth: number,
  from: number, to: number, points = 160,
) {
  if (!(sigma > 0) || !isFinite(sigma)) return []
  const step = (to - from) / (points - 1)
  const out: { x: number; curve: number }[] = []
  for (let i = 0; i < points; i++) {
    const x = from + i * step
    // The distribution has no support below zero.
    const pdf = x <= 0 ? 0 : (x / (sigma * sigma)) * Math.exp(-(x * x) / (2 * sigma * sigma))
